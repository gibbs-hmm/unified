...
#ifndef comp_bio_protein_fasta
#define comp_bio_protein_fasta
#include <iostream>
#include <fstream>
#include <string>
#include <vector>
...
public:
  ...
  protein_fasta_sequence(vector<string> &options) 
  {
    language_size = 20.0;
    mark_ct = 0;
    total_ct = 0;
  ...
  void initialize(string seq_name, string sequence, int markov_val) 
    {
      ...
      pseudo_counts = vector<double>((int)mark_ct,1.0);
      mark_count = vector<double>((int)mark_ct,0.0);
      totals = vector<double>((int)total_ct,0.0);

      pseudo_total = language_size;
      // Calculate Limit
      limit = pow((double)10.0,(double)100.0);

      // Calculate Seg_stop
      if(!seg_stop) {
	if(actual.length() > 2000)
	  seg_stop = min((int)(actual.length()/200),(int)199);
	else 
	  {
	    seg_stop = min((int)(actual.length()/(markov+2)),(int)199);
	    seg_stop /=8;
	  }
	seg_stop++;
...
class protein_fasta: public language {
private:
  vector<protein_fasta_sequence> sequences;
...
public:
  protein_fasta() {}
  protein_fasta(string file, int markov_val, vector<string> o) 
  {
    options = o;
    markov = markov_val;
    fileName = file;
  ...
  void initialize() 
  {
    ...
    sequences = vector<protein_fasta_sequence>(nbr_sequences);
    nbr_of_seq = 0;
    infile.clear();
    infile.seekg(0);
    ...
    while(infile.getline(temp,512))
      {
	if(temp[0] == '>') {
	  sequences[nbr_of_seq] = protein_fasta_sequence(options);

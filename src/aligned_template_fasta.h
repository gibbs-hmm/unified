...
#ifndef comp_bio_aligned_templated_fasta
#define comp_bio_aligned_templated_fasta
#include <iostream>
#include <fstream>
#include <string>
#include <vector>
...
template <class T>
class aligned_templated_fasta_sequence {
private:
...
public:
  aligned_templated_fasta_sequence() 
  {
    language = T();
  }
  aligned_templated_fasta_sequence(vector<string> &options) 
  {
    language = T();
    raw = 0;
    mark_ct = 0;
...
template <class T>
class aligned_templated_fasta: public language {
private:
  vector<aligned_templated_fasta_sequence<T> > sequences;
  ...
  int prior;
public:
  aligned_templated_fasta() {}
  aligned_templated_fasta(string file, int markov_val, vector<string> o) 
  {   
    alignment_value = 2;
    prior = 0;
    vector<string>::iterator p;
	...
	else if((*p).compare(0,2,"pa",0,2) == 0) {
	  prior = 0; 
	}
  ...
  void initialize() 
  {
	...
	if(temp[0] == '>') {
	  if(seq_counter != alignment_value) {
	    ...
	    names.push_back(temp_name);
	    seqs.push_back(temp_seq);
	    sequences.push_back(aligned_templated_fasta_sequence<T>(options));
